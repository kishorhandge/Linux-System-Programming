/*
    ...
    --> Uses write() to write data and fdatasync() to flush only the file data (excluding metadata) to disk.
    --> Demonstrates that the process may wait until data is flushed.
    --> Error handling is done using errno and strerror().
*/

#include<stdio.h>                                       // For printf()
#include<unistd.h>                                      // For write(), close(), and fdatasync()
...
int main()
{
    ...
    fdatasync(fd);                                              // Flush file data (excluding metadata) to disk; process may wait
